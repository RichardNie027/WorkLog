...
+ (void)showMessage:(NSString * _Nullable)message
    ...
    [[self sharedInstance] showToastWithType:ToastTypeWords
                                     message:message
                                      inView:inView
                                     originY:0
                                       image:nil
...
+ (void)showImage:(UIImage * _Nullable)image
         ...
         duration:(NSTimeInterval)duration
    finishHandler:(dispatch_block_t _Nullable)handler {
    [[self sharedInstance] showToastWithType:ToastTypeImage
                                     message:message
                                      inView:nil
                                     originY:0
                                       image:image
...
- (void)showToastWithType:(ToastType)type
                  message:(NSString * _Nullable)message
    ...
    self.toastView = [ToastView toastWithMessage:message
                                              type:type
                                           originY:originY
                                          tipImage:image];
    self.toastView.alpha = 0;
    UIView *containerView = nil == inView ? toast_currentWindow() : inView;
